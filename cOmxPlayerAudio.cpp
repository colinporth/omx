...
bool cOmxPlayerAudio::Decode (OMXPacket *pkt) {
  ...
  cLog::Log (LOGINFO, "OMXPlayerAudio::Decode dts:%.0f pts:%.0f size:%d", pkt->dts, pkt->pts, pkt->size);
...
bool cOmxPlayerAudio::OpenDecoder() {
  ...
  bAudioRenderOpen = m_decoder->Initialize (
    m_av_clock, m_config, m_pAudioCodec->GetChannelMap(), m_pAudioCodec->GetBitsPerSample());
